			...
			<div className="flex min-h-screen flex-col px-4 pb-32 pt-4">
				<main className="mx-auto w-full max-w-lg space-y-6">
					...
					<section>
						...
						<div className="-mx-4 mt-2 flex gap-3 overflow-x-auto px-0 pb-2 after:block after:w-4 after:shrink-0 after:content-[''] [scrollbar-width:none] [-webkit-overflow-scrolling:touch] [&::-webkit-scrollbar]:hidden">
							{exchanges.length === 0 ? (
								<div className="ml-4 box-border flex w-[calc(100%-2rem)] shrink-0 flex-col items-center justify-center rounded-2xl border border-primary/20 bg-white/60 py-12 backdrop-blur-md">
								...
								exchanges.map((ex) => {
									...
									return (
										<Link
											key={ex.id}
											href={`/exchange/${ex.id}`}
											className={`flex w-56 min-w-56 flex-none flex-col overflow-hidden rounded-2xl border border-primary/20 p-4 shadow-sm backdrop-blur-md transition-opacity first:ml-4 hover:opacity-90 ${
							...
							{books.length === 0 ? (
								...
								books.map((book) => (
									<Link
										key={book.id}
										href={`/book/${book.id}`}
										className="flex w-28 min-w-28 flex-none flex-col overflow-hidden rounded-xl border border-primary/20 bg-white/60 shadow-sm backdrop-blur-md transition-opacity first:ml-4 hover:opacity-90"
									>
